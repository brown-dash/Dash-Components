...
export const ListBox = (props: IListBoxProps) => {
  const { items, isOpen, setIsOpen, maxItems, backgroundColor, selectedItem, setSelectedItem, filter, hasShadow } = props

  const getListItem = (item: IListBoxItemProps, ind: number, selected: boolean):JSX.Element => {
    return (
      <ListItem 
        key={item.val + ind} 
        val={item.val} 
  ...
  return (
    isOpen ? <div className="listBox-container" 
      style={{
        height: maxItems && itemElements.length >= maxItems  ? maxItems * 30 : undefined,
